...
let uniqueFournisseurs = [...new Set(props.mes_licences.flatMap(licence => licence.fournisseur))];

const form = useForm({
    table: "commande_products",
    colonnes: ['fournisseur'],
    format: "licences",
    fournisseur: uniqueFournisseurs.map(fournisseur => ({
        title: fournisseur,
        selected: false
    }))
});
const filtersBy = ref([
    {
        title: 'Fournisseur', value: "fournisseur", icon_purple: "/images/tag_purple.svg", icon_black: "/images/tag.svg",
        options: form.fournisseur
    }
]);
const updateSelection = async (filterTitle, optionTitle) => {
    ...
    try {
        ...
        props.mes_licences = response.data;
    } catch (error) {
        console.error(error);
    }
}
