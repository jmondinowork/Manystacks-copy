...
let uniqueSousCategories = [...new Set(props.mes_equipements.map(equipement => equipement.sous_categorie))];
const form = useForm({
    table: "commande_products",
    colonnes: ['sous_categorie', 'status'],
    sous_categorie: uniqueSousCategories.map(sous_categorie => ({
        title: sous_categorie,
        selected: false
    })),
    status: [
        { title: "En service", selected: false },
        { title: "En reserve", selected: false },
        { title: "En maintenance", selected: false },
        { title: "Hors service", selected: false }
    ]
});
const filtersBy = ref([
    {
    ...
    {
        title: 'Statut', value: "status", icon_purple: "/images/statut_purple.svg", icon_black: "https://assets-global.website-files.com/65474f5b3ac46dc4b33db7b7/65d72913a33625943458fc24_Vectors-Wrapper.svg",
        options: form.status
    }
]);
