...
const productPriceLocation = (item) => {
    if (item.type_contrat === 'location') {
        return item.prix ? item.prix : item.product.prix_location;
    }
};
...
const totalLocationMonth = computed(() => {
    return data.commandes
        ...
        .reduce((acc, item) => acc + ((item.prix ? item.prix : item.product.prix_location) * item.quantity), 0)
        .toFixed(2);
});
                ...
                <div v-for="commande in data.commandes" :key="commande.id" class="w_container vertical gap12px">
                    <div class="w_container vertical gap16px white round padding12px">
                        ...
                        <div class="w_container justifyspacebetween">
                            ...
                            <div class="w_container alignend" v-if="commande.type_contrat === 'location'">
                                <div class="text20px unbounded">{{ productPriceLocation(commande) }} €</div>
